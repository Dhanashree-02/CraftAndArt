...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    // Get user email based on order_id
    $userQuery = "SELECT users.email, users.name, orders.user_id FROM orders
    ...
    if ($user) {
        ...
        if ($updateStmt->execute()) {
            ...
            try {
                ...
                $mail->Body = "Hello,<strong>$user_name</strong><br><br>Your order with ID <strong>$order_id</strong> has been updated to <strong>" . ucfirst($status) . "</strong>.<br>Thank you for shopping with us!<br><br>Regards,<br>CraftLoving Team";

                $mail->send();
            } catch (Exception $e) {
                $message .= " However, the email could not be sent. Mailer Error: {$mail->ErrorInfo}";
            }
        } else {
            // Error occurred
        ...
        $message = "User email not found for the order.";
    }
}
...
<head>
    <meta charset="utf-8">
    <title>Craft Loving | Admin Order Verification</title>
                            ...
                            <tbody>
                                <?php while ($order = $orderResult->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span
                                                class="text-secondary fs-6 badge badge-<?php echo getStatusBadgeClass($order['status']); ?>">
